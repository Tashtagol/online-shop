        ...
        <div class="cart-summary">
            <div class="cart-total">
                ...
                <strong id="cart-total">
                    <?= number_format(array_reduce($orderProducts, fn($sum, $p) => $sum + $p->getSum(), 0.0), 2, ',', ' ') ?> ₽
                </strong>
            </div>

            <a href="/order" class="cart-btn">Оформить заказ</a>
        </div>
    </div>
</section>
...
<style>
    ...
    .cart-desc { font-size: 15px; color: #4b5563; display: -webkit-box; -webkit-line-clamp: 3; -webkit-box-orient: vertical; overflow: hidden; }
    .cart-right { text-align: right; }
    .cart-price { font-size: 20px; font-weight: 800; color: #3b82f6; margin-bottom: 12px; }
    .cart-qty { display: inline-flex; align-items: center; gap: 10px; border: 1px solid #f3b4d3; border-radius: 12px; padding: 6px 10px; }
    .cart-qty button { width: 26px; height: 26px; border: none; border-radius: 8px; background: #fde2f0; cursor: pointer; }
    .qty-value { min-width: 22px; text-align: center; font-weight: 600; }
    .cart-summary { display: flex; justify-content: space-between; margin-top: 40px; border-top: 2px dashed #f3b4d3; padding-top: 20px; }
    .cart-total strong { font-size: 26px; color: #3b82f6; }
    .cart-btn { padding: 16px 56px; border: none; border-radius: 16px; background: linear-gradient(135deg, #ec4899, #f472b6); color: #fff; font-weight: 700; cursor: pointer; }
    .btn-clear { margin-bottom: 20px; padding: 10px 20px; background: #f87171; color: white; border: none; border-radius: 12px; cursor: pointer; font-weight: 700; }
    ...
    .btn-cancel { background: #e5e7eb; border: none; padding: 10px 16px; border-radius: 10px; cursor: pointer; }
    .btn-confirm { background: #ef4444; color: white; border: none; padding: 10px 16px; border-radius: 10px; cursor: pointer; }
</style>
